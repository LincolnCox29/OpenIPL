...
OpenIPLErrorInfo writeImg(const char* path, Img img)
{
    ...
    if (ext != NULL)
    {
        if (strcmp(ext, ".png") == 0 || strcmp(ext, ".PNG") == 0)
        {
            ...
            return (OpenIPLErrorInfo) { OIPL_SUCCESS, NULL };
        }
        ...
        else if (strcmp(ext, ".tga") == 0 || strcmp(ext, ".TGA") == 0)
        {
            ...
            return (OpenIPLErrorInfo) { OIPL_ERROR_UNSUPPORTED_IMAGE_FORMAT, "Unsupported image format." };
    }
    else
        return (OpenIPLErrorInfo) { OIPL_ERROR_FILE_PATH_DOES_NOT_EXIST, "File path does not have an extension!" };
}
